          ...
          <div ref={chatBodyRef} className="scrollbar-none h-[400px] space-y-4 overflow-y-auto p-4">
            {chatHistory.map((chat, index) => (
              <div key={index} className={`flex flex-col ${chat.role === "model" ? "items-start" : "items-end"} gap-3`}>
                ...
                <p
                  className={`max-w-[85%] whitespace-pre-line rounded-lg px-5 py-4 text-base text-white ${
                    chat.role === "model" ? "rounded-tl-none bg-gray-600" : "rounded-tr-none bg-gray-500"
                  } overflow-hidden`}
                >
                  {chat.text}
                </p>
